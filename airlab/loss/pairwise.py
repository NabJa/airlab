    ...
    def GetCurrentMask(self, displacement):
        ...
        if not self._moving_mask is None:
            self._warped_moving_mask = F.grid_sample(
                self._moving_mask.image, displacement, align_corners=False
            )
...
class MSE(_PairwiseImageLoss):
    ...
    def forward(self, displacement):
        ...
        self.warped_moving_image = F.grid_sample(
            self._moving_image.image, displacement, align_corners=False
        )
...
class NCC(_PairwiseImageLoss):
    ...
    def __init__(self, fixed_image, moving_image, fixed_mask=None, moving_mask=None):
        super(NCC, self).__init__(
            fixed_image, moving_image, fixed_mask, moving_mask, False, False
        )
    ...
    def forward(self, displacement):
        ...
        value = (
            -1.0
            * th.sum(
                (fixed_image_valid - th.mean(fixed_image_valid))
                * (moving_image_valid - th.mean(moving_image_valid))
            )
            / th.sqrt(
...
class LCC(_PairwiseImageLoss):
    def __init__(
        ...
        if kernel_type == "box":
            ...
            self._kernel = th.ones(
                *kernel_size.tolist(), dtype=self._dtype, device=self._device
            ) / float(np.product(kernel_size) ** 2)
        elif kernel_type == "gaussian":
            self._kernel = utils.gaussian_kernel(
                sigma, dim, asTensor=True, dtype=self._dtype, device=self._device
            )
        ...
        if dim == 2:
            ...
            self._variance_fixed_image = F.conv2d(
                self._fixed_image.image.pow(2), self._kernel
            ...
            self._variance_fixed_image = F.conv3d(
                self._fixed_image.image.pow(2), self._kernel
            ) - (self._mean_fixed_image.pow(2))
    ...
    def _lcc_loss_2d(self, warped_image, mask):
        ...
        variance_moving_image = F.conv2d(warped_image.pow(2), self._kernel) - (
            mean_moving_image.pow(2)
        )

        mean_fixed_moving_image = F.conv2d(
            self._fixed_image.image * warped_image, self._kernel
        )

        cc = (
            mean_fixed_moving_image - mean_moving_image * self._mean_fixed_image
        ) ** 2 / (variance_moving_image * self._variance_fixed_image + 1e-10)
    ...
    def _lcc_loss_3d(self, warped_image, mask):
        ...
        variance_moving_image = F.conv3d(warped_image.pow(2), self._kernel) - (
            mean_moving_image.pow(2)
        )

        mean_fixed_moving_image = F.conv3d(
            self._fixed_image.image * warped_image, self._kernel
        )
...
class MI(_PairwiseImageLoss):
    ...
    def __init__(
        ...
        self._bins_fixed_image = th.linspace(
            self._background_fixed,
            self._max_f,
            self.bins,
        ...
        self._bins_moving_image = th.linspace(
            self._background_moving,
            self._max_m,
            self.bins,
            device=fixed_image.device,
            dtype=fixed_image.dtype,
    ...
    def forward(self, displacement):
        ...
        sample = (
            th.zeros(
                ...
                dtype=self._fixed_image.dtype,
            ).uniform_()
            < self._spatial_samples
        )
...
class NGF(_PairwiseImageLoss):
    ...
    def __init__(
        ...
        if self._dim == 2:
            dx = (
                fixed_image.image[..., 1:, 1:] - fixed_image.image[..., :-1, 1:]
            ) * fixed_image.spacing[0]
            dy = (
                fixed_image.image[..., 1:, 1:] - fixed_image.image[..., 1:, :-1]
            ) * fixed_image.spacing[1]
            ...
            dx = (
                fixed_image.image[..., 1:, 1:, 1:] - fixed_image.image[..., :-1, 1:, 1:]
            ) * fixed_image.spacing[0]
            dy = (
                fixed_image.image[..., 1:, 1:, 1:] - fixed_image.image[..., 1:, :-1, 1:]
            ) * fixed_image.spacing[1]
    ...
    def _ngf_loss_2d(self, warped_image):
        ...
        dx = (
            warped_image[..., 1:, 1:] - warped_image[..., :-1, 1:]
        ) * self._moving_image.spacing[0]
        dy = (
            warped_image[..., 1:, 1:] - warped_image[..., 1:, :-1]
        ) * self._moving_image.spacing[1]
    ...
    def forward(self, displacement):
        ...
        for dim in range(self._dim):
            value = (
                value + ng_warped_image[:, dim, ...] * self._ng_fixed_image[:, dim, ...]
            )
...
class SSIM(_PairwiseImageLoss):
    ...
    def __init__(
        ...
        if kernel_type == "box":
            kernel_size = sigma * 2 + 1
            self._kernel = th.ones(*kernel_size.tolist()) / float(
                np.product(kernel_size) ** 2
            )
        elif kernel_type == "gaussian":
    ...
    def forward(self, displacement):
        ...
        variance_moving_image = F.conv2d(
            self._warped_moving_image.pow(2), self._kernel
        ) - (mean_moving_image.pow(2))
        ...
        contrast = (
            2
            * th.sqrt(self._variance_fixed_image + 1e-10)
            * th.sqrt(variance_moving_image + 1e-10)
            + self._c2
        ...
        sim = (
            luminance.pow(self._alpha)
            * contrast.pow(self._beta)
            * structure.pow(self._gamma)
        )
